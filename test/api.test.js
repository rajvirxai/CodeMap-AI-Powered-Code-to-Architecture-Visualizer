...
test('CodeMap Backend API Tests', async (t) => {
  ...
  // Test 6: GET /architecture/:folderId (Non-existent folderId)
  await t.test('GET /architecture/:folderId returns 404 for non-existent ID', async () => {
    const res = await fetch(`${baseUrl}/architecture/non-existent-id-12345`);
    assert.strictEqual(res.status, 404);
